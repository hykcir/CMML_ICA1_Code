...
for (a in c(0.005, 0.01, 0.015, 0.02)) {
  ...
  for (reps in 1:Nruns){
    ...
    # clean_pm$Reps <- reps
    alpha_pms[[name]] <- rbind(alpha_pms[[name]], clean_pm_dc, clean_pm_pc,
                               clean_pm_comb)
  }
...
for (g in seq(0.75, 0.95, by = 0.05)) {
  ...
  for (reps in 1:Nruns){
    ...
    clean_pm_comb$Reps <- reps
    # clean_pm$Reps <- reps
    gamma_pms[[name]] <- rbind(gamma_pms[[name]], clean_pm_dc, clean_pm_pc,
                               clean_pm_comb)
  }
...
for (b in c(0.5, 1, 6, 9, 12)) {
  ...
  for (reps in 1:Nruns){
    ...
    clean_pm_comb$Reps <- reps
    # clean_pm$Reps <- reps
    beta_pms[[name]] <- rbind(beta_pms[[name]], clean_pm_dc, clean_pm_pc,
                              clean_pm_comb)
  }
